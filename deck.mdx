...
import { Notes, Footer, Image, Appear } from "mdx-deck";
...
```elixir subtitle="Tell the tracer where to send traces" 3[42:56]
iex(1)> pid = Trace.start()
#PID<0.118.0>
iex(2)> :erlang.trace(:all, true, [:call, {:tracer, pid}])
59
...
iex(1)> :dbg.fun2ms(fn [m, n] when n > 3 -> return_trace() end)
```
...
** (CompileError) iex:1: undefined function return_trace/0
    (elixir 1.10.2) src/elixir_fn.erl:15: anonymous fn/4 in :elixir_fn.expand/3
    (stdlib 3.12.1) lists.erl:1354: :lists.mapfoldl/3
    (elixir 1.10.2) src/elixir_fn.erl:20: :elixir_fn.expand/3
    (stdlib 3.12.1) lists.erl:1354: :lists.mapfoldl/3
```
...
iex(1)> :dbg.fun2ms(fn [m,  n] when m > 3 -> :return_trace end)
      > |> put_in([Access.at(0), Access.elem(2), Access.at(0)], {:return_trace})
[{[:"$1",:"$2"],[{:>,:"$2",3}],[{:return_trace}]}]
```
